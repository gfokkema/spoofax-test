...
//	final static String sourcePath = "test.nabl";
	...
	public void run(Spoofax spoofax) throws InterruptedException, MetaborgException {
		...
		// Initalize project
		IProject project = spoofax.injector
				.getInstance(SimpleProjectService.class)
				.create(spoofax.resourceService.resolve("/tmp/project"));
		...
		spoofax.builder
				.build(build, new NullProgressReporter(), new CancellationToken())
				.transformResults()
				.forEach(t -> System.out.println("Output: " + t.ast()));
	}
}
